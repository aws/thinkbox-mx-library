...
/**
 * Possibly applies a ticks_to_seconds_particle_istream decorator in to a particle_istream if it contains time channel
 * that does not have a floating point datatype.
 * @param pin The stream to potentially modify
...
convert_time_channels_to_seconds( frantic::particles::particle_istream_ptr pin );

/**
 * This function will populate 'outMap' with the same channels as 'requestedChannels' except it will convert time
 * channels (ie. Age and LifeSpan) to TimeValue ticks.
 ...
 * data types changed to TimeValue.
 */
void convert_time_channels_to_ticks( const frantic::channels::channel_map& requestedChannels,
                                     frantic::channels::channel_map& outMap );

/**
 * This particle_istream decorator will convert time channels (ie. "Age" and "LifeSpan") from 3ds Max TimeValue ticks to
 * float32 seconds. If the underlying stream is already in floating point time format, no conversion is applied.
 */
class ticks_to_seconds_particle_istream : public frantic::particles::streams::delegated_particle_istream {
  public:
    ticks_to_seconds_particle_istream( frantic::particles::particle_istream_ptr delegateStream/*, frantic::channels::data_type_t preferredFloatTimeType = frantic::channels::data_type_float32*/ );

    virtual ~ticks_to_seconds_particle_istream();
  ...
  private:
    frantic::channels::channel_accessor<TimeValue> m_inAgeAccessor, m_inLifeSpanAccessor;
    frantic::channels::channel_cvt_accessor<double> m_outAgeAccessor, m_outLifeSpanAccessor;
    frantic::channels::channel_map m_outMap, m_nativeMap;

    // HACK: I have currently hard-coded 'm_preferredFloatTimeType' to float32 since it allows easy conversion without
    // allocating temporary space since sizeof(float32) == sizeof(TimeValue)
    //       If we want to allow an arbitrary type, we need to a channel_map_adaptor and allocate temporary storage for
    //       the particle data coming from the delegate.
    frantic::channels::data_type_t
        m_preferredFloatTimeType; // Set by the client, to indicate the native type of the new time channels.
};

inline ticks_to_seconds_particle_istream::ticks_to_seconds_particle_istream(
    frantic::particles::particle_istream_ptr
        delegateStream /*, frantic::channels::data_type_t preferredFloatTimeType*/ )
    : delegated_particle_istream( delegateStream )
    , m_preferredFloatTimeType( frantic::channels::data_type_float32 ) {
    ...
    for( std::size_t i = 0, iEnd = delegateNativeMap.channel_count(); i < iEnd; ++i ) {
        ...
        if( ( ch.name() == _T("Age") || ch.name() == _T("LifeSpan") ) &&
            ...
            m_nativeMap.define_channel( ch.name(), ch.arity(), m_preferredFloatTimeType, ch.offset() );
        } else {
...
inline void ticks_to_seconds_particle_istream::set_channel_map_impl( const frantic::channels::channel_map& newMap ) {
    ...
    for( std::size_t i = 0, iEnd = newMap.channel_count(); i < iEnd; ++i ) {
        ...
        // We can change the type without adjusting the offsets since sizeof(TimeValue) == sizeof(float32)
        // We only do the conversion if the delegate stream has non-float time, and the requested stream has float time.
        if( ( ch.name() == _T("Age") || ch.name() == _T("LifeSpan") ) && delegateNativeMap.has_channel( ch.name() ) &&
            ...
            delegateMap.define_channel( ch.name(), ch.arity(),
    ...
    if( newMap.has_channel( _T("Age") ) && delegateMap.has_channel( _T("Age") ) &&
        ...
        m_inAgeAccessor = delegateMap.get_accessor<TimeValue>( _T("Age") );
        m_outAgeAccessor = newMap.get_cvt_accessor<double>( _T("Age") );
    }
    ...
    if( newMap.has_channel( _T("LifeSpan") ) && delegateMap.has_channel( _T("LifeSpan") ) &&
        ...
        m_inLifeSpanAccessor = delegateMap.get_accessor<TimeValue>( _T("LifeSpan") );
        m_outLifeSpanAccessor = newMap.get_cvt_accessor<double>( _T("LifeSpan") );
    }
}

inline void ticks_to_seconds_particle_istream::process_particle( char* pParticle ) const {
    if( m_outAgeAccessor.is_valid() ) { // We only set the accessor if m_inAgeAccessor is also valid
        m_outAgeAccessor.set( pParticle, frantic::max3d::to_seconds<double>( m_inAgeAccessor.get( pParticle ) ) );
    }

    if( m_outLifeSpanAccessor.is_valid() ) { // We only set the accessor if m_inLifeSpanAccessor is also valid
        m_outLifeSpanAccessor.set( pParticle,
                                   frantic::max3d::to_seconds<double>( m_inLifeSpanAccessor.get( pParticle ) ) );
    }
}

inline void ticks_to_seconds_particle_istream::process_particles( const tbb::blocked_range<std::size_t>& range,
...
inline const frantic::channels::channel_map& ticks_to_seconds_particle_istream::get_channel_map() const {
    return m_outMap;
}

inline const frantic::channels::channel_map& ticks_to_seconds_particle_istream::get_native_channel_map() const {
    return m_nativeMap;
}

inline void ticks_to_seconds_particle_istream::set_channel_map( const frantic::channels::channel_map& newMap ) {
    this->set_channel_map_impl( newMap );
}

inline void ticks_to_seconds_particle_istream::set_default_particle( char* rawParticleBuffer ) {
    ...
    if( m_outAgeAccessor.is_valid() && m_inAgeAccessor.is_valid() )
        m_inAgeAccessor.get( rawParticleBuffer ) = SecToTicks( m_outAgeAccessor.get( rawParticleBuffer ) );

    if( m_outLifeSpanAccessor.is_valid() && m_inLifeSpanAccessor.is_valid() )
        m_inLifeSpanAccessor.get( rawParticleBuffer ) = SecToTicks( m_outLifeSpanAccessor.get( rawParticleBuffer ) );

    delegated_particle_istream::set_default_particle( rawParticleBuffer );
}

inline bool ticks_to_seconds_particle_istream::get_particle( char* rawParticleBuffer ) {
...
inline bool ticks_to_seconds_particle_istream::get_particles( char* particleBuffer, std::size_t& numParticles ) {
    ...
    tbb::parallel_for( tbb::blocked_range<std::size_t>( 0, numParticles ),
                       boost::bind( &ticks_to_seconds_particle_istream::process_particles, this, _1, particleBuffer ),
...
convert_time_channels_to_seconds( frantic::particles::particle_istream_ptr pin ) {
    ...
    if( ( nativeMap.has_channel( _T("Age") ) &&
        ...
        pin.reset( new frantic::max3d::particles::streams::ticks_to_seconds_particle_istream( pin ) );
    }

    return pin;
}

inline void convert_time_channels_to_ticks( const frantic::channels::channel_map& requestedChannels,
                                            frantic::channels::channel_map& outMap ) {
    outMap.reset();
    for( std::size_t i = 0, iEnd = requestedChannels.channel_count(); i < iEnd; ++i ) {
        ...
        if( ( ch.name() == _T("Age") || ch.name() == _T("LifeSpan") ) &&
            frantic::channels::is_channel_data_type_float( ch.data_type() ) ) {
            outMap.define_channel( ch.name(), ch.arity(),
                                   frantic::channels::channel_data_type_traits<TimeValue>::data_type(), ch.offset() );
        } else {
